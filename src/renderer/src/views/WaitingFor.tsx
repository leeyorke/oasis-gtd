    ...
    <div className="composition">
      ...
      >
          ...
          <div className="task-list">
            {waitingItems.length === 0 ? (
              <div style={{
                ...
                Nothing pending. You are in control.
              </div>
            ) : (
              waitingItems.map((item, i) => (
                <div
                  key={item.id}
                  className="waiting-row fade-in"
        ...
        <footer className="sheet-footer">
          <div className="footer-logo">Wait</div>
          <div className="footer-block">
            <div className="footer-title">Pending</div>
            <div className="footer-text">{waitingItems.length} items</div>
          </div>
          <div className="footer-block">
            <button className="btn-text" onClick={() => setShowForm(true)} style={{ paddingLeft: 0 }}>
              + Add Item
            </button>
          </div>
        </footer>
      ...
      {/* ─── Blue Sheet: Waiting visualization ──────────────────── */}
      <div
      ...
      >
        <div style={{ position: 'relative', zIndex: 3, padding: '2rem', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div style={{ fontFamily: 'var(--font-sans)', fontSize: '0.55rem', textTransform: 'uppercase', letterSpacing: '0.15em', color: 'rgba(244,243,239,0.5)' }}>
            Waiting Summary
          </div>

          {/* Simple aging visualization */}
            ...
            {waitingItems.slice(0, 4).map(item => {
              ...
              return (
                <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ fontFamily: 'var(--font-sans)', fontSize: '0.68rem', color: urgency, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '70%' }}>
                    {item.title}
                  </div>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.1rem', color: urgency }}>
                    {days}d
                  </div>
                </div>
              )
          ...
          <div style={{ fontFamily: 'var(--font-sans)', fontSize: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.15em', color: 'rgba(244,243,239,0.3)' }}>
            {waitingItems.length} items tracked
          </div>
        </div>
      </div>

      {/* ─── White Card: Add Waiting ─────────────────────────────── */}
        ...
        {showForm ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', flex: 1, WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
            <textarea
              ...
              style={{ fontSize: '1.2rem', marginTop: 0, flex: '0 0 60px', resize: 'none' }}
            />
            <input
              placeholder="Waiting for (person/team)"
              value={form.waiting_for}
              onChange={e => setForm(f => ({ ...f, waiting_for: e.target.value }))}
              style={{ background: 'transparent', border: 'none', borderBottom: '1px solid rgba(20,28,58,0.1)', outline: 'none', fontFamily: 'var(--font-sans)', fontSize: '0.75rem', color: 'var(--ink-secondary)', padding: '0.3rem 0', width: '100%' }}
            />
            <input
              type="date"
              value={form.since}
            ...
            <div className="capture-actions" style={{ marginTop: 'auto' }}>
              <button className="btn-text" onClick={handleAdd}>Add</button>
              <button className="btn-text" style={{ marginLeft: 'auto' }} onClick={() => setShowForm(false)}>Cancel</button>
            </div>
          </div>
        ) : (
          <>
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: '3.5rem', color: 'rgba(20,28,58,0.08)', lineHeight: 1 }}>
                  {waitingItems.length}
                </div>
                <div style={{ fontFamily: 'var(--font-sans)', fontSize: '0.55rem', textTransform: 'uppercase', letterSpacing: '0.15em', color: 'var(--ink-secondary)' }}>
                  pending
                </div>
              </div>
            </div>
            <div className="capture-actions">
              <button className="btn-text" onClick={() => setShowForm(true)}>+ Track Item</button>
            </div>
          </>
        )}

        <div className="emboss-text">wait</div>
      </div>
    </div>
  )
}
