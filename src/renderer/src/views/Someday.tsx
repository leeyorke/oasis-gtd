...
const HORIZONS: { id: Horizon; label: string; description: string }[] = [
  { id: 'soon', label: 'Soon', description: 'This week or next' },
  { id: '1month', label: '1 Month', description: 'Within 30 days' },
  { id: '3months', label: '3 Months', description: 'This quarter' },
  { id: '1year', label: '1 Year', description: 'This year' },
  { id: 'someday', label: 'Someday', description: 'No timeline' },
]
...
export default function Someday() {
  ...
  const total = somedayItems.length
    ...
    <div className="composition">
      ...
      >
            ...
            return (
              <div key={h.id} className="horizon-section fade-in">
                ...
                {items.map((item, i) => (
                  <div
                    key={item.id}
                    className="someday-item"
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.65rem 0', borderBottom: '1px solid rgba(20,28,58,0.04)', animationDelay: `${i * 0.04}s` }}
                  >
                    <div>
                      <div className="task-title" style={{ fontSize: '1.2rem' }}>{item.title}</div>
                      {item.notes && (
                        <div className="task-meta" style={{ marginTop: '0.2rem' }}>{item.notes}</div>
                      )}
                    </div>
                    <button className="delete-btn" onClick={() => removeSomeday(item.id)} style={{ opacity: 0, transition: 'opacity 0.2s' }}
        ...
        <footer className="sheet-footer">
          <div className="footer-logo">Dream</div>
          <div className="footer-block">
            <div className="footer-title">Horizons</div>
            <div className="footer-text">{total} ideas captured</div>
          </div>
          <div className="footer-block">
      ...
      >
        ...
        <div style={{ position: 'absolute', bottom: '1.5rem', left: '2rem', zIndex: 4 }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.4rem', color: 'rgba(244,243,239,0.6)', fontStyle: 'italic', lineHeight: 1.2 }}>
            The future belongs to those<br />who prepare for it today.
          </div>
          <div style={{ fontFamily: 'var(--font-sans)', fontSize: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.15em', color: 'rgba(244,243,239,0.3)', marginTop: '0.5rem' }}>
            Someday / Maybe
          </div>
        </div>
      </div>
        ...
        {showForm ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem', flex: 1 }}>
            <textarea
              ...
              style={{ fontSize: '1.2rem', marginTop: 0, flex: '0 0 70px', resize: 'none', WebkitAppRegion: 'no-drag' } as React.CSSProperties}
              autoFocus
            />
            <select
              value={horizon}
            ...
            >
              {HORIZONS.map(h => (
                <option key={h.id} value={h.id}>{h.label} — {h.description}</option>
              ))}
            </select>
            <div className="capture-actions" style={{ marginTop: 'auto' }}>
              <button className="btn-text" onClick={handleAdd}>Capture</button>
          ...
          <>
            ...
            <div className="capture-actions">
              <button className="btn-text" onClick={() => setShowForm(true)}>+ New Idea</button>
            </div>
          </>
        )}

        <div className="emboss-text">dream</div>
      </div>
    </div>
  )
}
